...
bool Convert(const ScorePtr &score_ptr, ScoreDetails &score_details)
{
    ...
    if (score_details_ptr->has_food_used()) { Convert(score_details_ptr->food_used(), score_details.food_used); }
...
{
    score = Score();
    if (observation_ptr.HasErrors()) { return false; }
...
bool Convert(const ObservationRawPtr &observation_raw, UnitPool &unit_pool, uint32_t game_loop)
{
    for (int i = 0; i < observation_raw->units_size(); ++i) {
        ...
        if (!Convert(observation_unit.display_type(), unit->display_type)) { return false; }
        if (!Convert(observation_unit.alliance(), unit->alliance)) { return false; }
        ...
        if (observation_unit.has_cloak()) {
            if (!Convert(observation_unit.cloak(), unit->cloak)) { return false; }
        } else {
        ...
        for (int passenger_index = 0; passenger_index < observation_unit.passengers_size(); ++passenger_index) {
            ...
            if (passengerProto.has_tag()) passengerUnit.tag = passengerProto.tag();
            if (passengerProto.has_health()) passengerUnit.health = passengerProto.health();
            if (passengerProto.has_health_max()) passengerUnit.health_max = passengerProto.health_max();
            if (passengerProto.has_shield()) passengerUnit.shield = passengerProto.shield();
            if (passengerProto.has_shield_max()) passengerUnit.shield_max = passengerProto.shield_max();
            if (passengerProto.has_energy()) passengerUnit.energy = passengerProto.energy();
            if (passengerProto.has_energy_max()) passengerUnit.energy_max = passengerProto.energy_max();
            if (passengerProto.has_unit_type()) passengerUnit.unit_type = passengerProto.unit_type();
...
bool Convert(const ObservationPtr &observation_ptr, RenderedFrame &render)
{
    ...
    if (observation_render.HasErrors()) { return false; }

    if (!observation_render->has_map()) { return false; }

    if (!Convert(observation_render->map(), render.map)) { return false; }

    if (!observation_render->has_minimap()) { return false; }

    if (!Convert(observation_render->minimap(), render.minimap)) { return false; }
...
static void ConvertSpatialAction(const SC2APIProtocol::ActionSpatial &action_proto, SpatialActions &actions)
{
    if (action_proto.has_unit_command()) {
        ...
        if (!Convert(action_select.type(), select.type)) return;
...
void ConvertFeatureLayerActions(const ResponseObservationPtr &response_observation_ptr, SpatialActions &actions)
{
    for (int i = 0; i < response_observation_ptr->actions_size(); ++i) {
        const SC2APIProtocol::Action &proto_action = response_observation_ptr->actions(i);
        if (!proto_action.has_action_feature_layer()) { continue; }
...
void ConvertRenderedActions(const ResponseObservationPtr &response_observation_ptr, SpatialActions &actions)
{
    for (int i = 0; i < response_observation_ptr->actions_size(); ++i) {
        const SC2APIProtocol::Action &proto_action = response_observation_ptr->actions(i);
        if (!proto_action.has_action_render()) { continue; }
...
{
    if (!options_proto.has_raw()) { return false; }
...
{
    if (!response_game_info_ptr->has_start_raw()) { return false; }
    const SC2APIProtocol::StartRaw &start_raw = response_game_info_ptr->start_raw();

    if (!start_raw.has_map_size() || !start_raw.map_size().has_x() || !start_raw.map_size().has_y()) { return false; }
    ...
    if (start_raw.has_pathing_grid()) { Convert(start_raw.pathing_grid(), game_info.pathing_grid); }

    if (start_raw.has_terrain_height()) { Convert(start_raw.terrain_height(), game_info.terrain_height); }

    if (start_raw.has_placement_grid()) { Convert(start_raw.placement_grid(), game_info.placement_grid); }
    ...
    for (const auto &player_info : response_game_info_ptr->player_info()) {
        game_info.player_info.push_back(sc2::PlayerInfo(static_cast<uint32_t>(player_info.player_id()),
    ...
    if (!response_game_info_ptr->has_options()) { return false; }

    if (!Convert(response_game_info_ptr->options(), game_info.options)) { return false; }
