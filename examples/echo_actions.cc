...
{
  public:
    void OnGameStart() override { last_echoed_gameloop_ = 0; }
    ...
    void OnStep() final
    {
        ...
        if (Control()->GetLastStatus() != SC2APIProtocol::Status::in_game) return;

        if (last_echoed_gameloop_ == obs->GetGameLoop()) return;
...
int main(int argc, char *argv[])
{
    ...
    while (coordinator.Update()) { sc2::SleepFor(10); }
