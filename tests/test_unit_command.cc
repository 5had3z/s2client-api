...
class TestAttackAttack : public TestUnitCommandTargetingPoint
{
  public:
    ...
    void SetTestTime() override { wait_game_loops_ = 50; }
};
...
class TestBehaviorCloakBanshee : public TestUnitCommandNoTarget
{
  public:
    ...
    {
        if (!test_unit_) { ReportError("Could not find the test unit."); }
...
class TestBehaviorSalvage : public TestUnitCommandNoTarget
{
  public:
    ...
    void SetTestTime() override { wait_game_loops_ = 200; }
};
...
class TestBuildBarracks : public TestUnitCommandTargetingPoint
{
  public:
    ...
    void SetTestTime() override { wait_game_loops_ = 150; }
...
class TestBuildBarracksTechLab : public TestUnitCommandNoTarget
{
  public:
    ...
    void OnTestFinish() override
    {
        ...
        if (barracks_units.size() != 1) { ReportError("Could not find landed barracks."); }

        // Find the tech lab.
        Units techlab_units = obs->GetUnits(Unit::Self, IsUnit(UNIT_TYPEID::TERRAN_BARRACKSTECHLAB));
        if (techlab_units.size() != 1) { ReportError("Could not find a tech lab."); }

        if (barracks_units.size() == 1 && techlab_units.size() == 1) {
            if (barracks_units[0]->add_on_tag == 0LL) { ReportError("Barracks does not have an add-on."); }
...
{
    void AdditionalTestSetup() override { target_point_ = origin_pt_; }
};
  ...
  public:
    ...
    void AdditionalTestSetup() override
    {
        ...
        agent_->Debug()->DebugCreateUnit(
          UNIT_TYPEID::PROTOSS_PYLON, origin_pt_, agent_->Observation()->GetPlayerID(), 1);
...
class TestBuildPylon : public TestUnitCommandTargetingPoint
{
  public:
    ...
    void OnTestFinish() override
    {
        ...
        VerifyUnitIdleAfterOrder(test_unit_type_);
        const ObservationInterface *obs = agent_->Observation();
        if (obs->GetPowerSources().size() != 1) { ReportError("Pylon is not being counted as a power source."); }
        KillAllUnits();
    }
};
...
class TestBuildSpineCrawler : public TestUnitCommandTargetingPoint
{
  public:
    ...
    void SetTestTime() override { wait_game_loops_ = 450; }
    ...
    void AdditionalTestSetup() override
    {
        ...
        agent_->Debug()->DebugCreateUnit(UNIT_TYPEID::ZERG_CREEPTUMORBURROWED,
          GetPointOffsetX(origin_pt_, -10),
...
class TestCancelBuildInProgressFactory : public TestUnitCommandNoTarget
{
  public:
    ...
    void SetTestTime() override { wait_game_loops_ = 100; }
    ...
    void OnStep() override
    {
        ...
        if (obs->GetGameLoop() < order_on_game_loop_) { return; }

        const Units &units = obs->GetUnits(Unit::Alliance::Self);
        for (const auto &unit : units) {
            if (unit->unit_type == test_unit_type_) { test_unit_ = unit; }
        }
        ...
        for (const auto &unit : units) {
            if (unit->unit_type == UNIT_TYPEID::TERRAN_FACTORY) { test_factory_ = unit; }
        }

        if (obs->GetGameLoop() < order_on_game_loop_ + 10) { return; }

        if (!orders_verified_ && factory_built_) { VerifyUnitOrders(test_unit_, ABILITY_ID::BUILD_FACTORY); }
...
class TestCancelTrainingQueue : public TestUnitCommandNoTarget
{
  public:
    ...
    void OnStep() override
    {
        ...
        if (!orders_verified_ && marine_trained_) { VerifyUnitOrders(test_unit_, ABILITY_ID::TRAIN_MARINE); }
...
class TestEffectHeal : public TestUnitCommandTargetingUnit
{
  public:
    ...
    void SetTestTime() override { wait_game_loops_ = 250; }

    void AdditionalTestSetup() override
    {
        agent_->Debug()->DebugCreateUnit(
          target_unit_type_, GetPointOffsetX(origin_pt_, 10), agent_->Observation()->GetPlayerID(), 1);
        agent_->Debug()->SendDebug();
    }
    ...
    void OnStep() override
    {
        ...
        for (const auto &unit : units) {
            if (unit->unit_type == target_unit_type_) { target_unit_ = unit; }
        }
        ...
        if (obs->GetGameLoop() < order_on_game_loop_ + 5) { return; }
        ...
        if (test_units_.size() > 0) { test_unit_ = test_units_.front(); }
...
class TestEffectScan : public TestUnitCommandTargetingPoint
{
  public:
    ...
    void SetTestTime() override { wait_game_loops_ = 300; }
    ...
    void OnStep() override
    {
        ...
        if (!hatchery_spawned_) {
            agent_->Debug()->DebugCreateUnit(
              UNIT_TYPEID::ZERG_HATCHERY, target_point_, agent_->Observation()->GetPlayerID() + 1, 1);
            agent_->Debug()->SendDebug();
        ...
        for (const auto &unit : units) {
            if (unit->unit_type == UNIT_TYPEID::ZERG_HATCHERY) { test_hatchery_ = unit; }
        }
        ...
        if (obs->GetGameLoop() < order_on_game_loop_ + 15) { return; }
        ...
        if (!verify_post_scan_) {
            ...
            if (test_hatchery_->is_on_screen == true) { ReportError("Enemy structure on screen is true."); }
            if (test_hatchery_->alliance != Unit::Alliance::Enemy) { ReportError("Enemy alliance is incorrect."); }
            if (test_hatchery_->owner != 2) { ReportError("Owner of unit is incorrect."); }
...
class TestHarvestProbeHarvestMinerals : public TestUnitCommandTargetingUnit
{
  public:
    ...
    void AdditionalTestSetup() override
    {
        ...
        agent_->Debug()->DebugCreateUnit(
          UNIT_TYPEID::PROTOSS_NEXUS, GetPointOffsetX(origin_pt_, -20), agent_->Observation()->GetPlayerID(), 1);
        agent_->Debug()->SendDebug();
    }
    ...
    void OnTestFinish() override
    {
        ...
        if (target_unit_->mineral_contents < 1400) { ReportError("Mineral patch does not contain any minerals."); }
    ...
    void OnTestFinish() override
    {
        ...
        // Ensure the vespene geyser has gas.
        {
            ...
            if (geysers.size() < 1) {
                ...
                if (geysers[0]->vespene_contents < 100) { ReportError("Geyser patch does not contain any vespene."); }
            }
        }
...
class TestEffectMoveMove : public TestUnitCommandTargetingPoint
{
  public:
    ...
    void AdditionalTestSetup() override { target_point_ = GetPointOffsetX(origin_pt_, -5); }
...
class TestRallyRally : public TestUnitCommandTargetingPoint
{
  public:
    ...
    void OnStep() override
    {
        ...
        if (obs->GetGameLoop() < order_on_game_loop_ + 20) { return; }
...
class TestResearchTerranInfantryArmorLevel2 : public TestUnitCommandNoTarget
{
  public:
    ...
    void AdditionalTestSetup() override { VerifyUpgradeOwned(test_upgrade_name_, true); }
...
{
  public:
    TestSensorTower() { test_unit_type_ = UNIT_TYPEID::TERRAN_SENSORTOWER; }
    ...
    {
        agent_->Debug()->DebugCreateUnit(
          UNIT_TYPEID::ZERG_MUTALISK, GetPointOffsetX(origin_pt_, 25), agent_->Observation()->GetPlayerID() + 1, 1);
        agent_->Debug()->DebugShowMap();
        agent_->Debug()->SendDebug();
    }
    ...
    void OnTestFinish() override
    {
        ...
        if (target_units.front()->is_blip != true) { ReportError("Target unit is not a blip."); }
        ...
        if (target_units.front()->owner != 0) { ReportError("Owner of unit is incorrect."); }
...
class TestShields : public TestUnitCommandNoTarget
{
  public:
    ...
    TestShields() { test_unit_type_ = UNIT_TYPEID::PROTOSS_ZEALOT; }
    ...
    void OnStep() override
    {
        ...
        if (!test_unit_damaged_) {
            if (test_unit->shield != 50) { ReportError("Pre-damage test unit shield is not correct."); }
            agent_->Debug()->DebugSetShields(25, test_unit_);
            agent_->Debug()->SendDebug();
            test_unit_damaged_ = true;
        }

        if (obs->GetGameLoop() < order_on_game_loop_ + 2) { return; }
...
class TestStopStop : public TestUnitCommandNoTarget
{
  public:
    ...
    void AdditionalTestSetup() override { target_point_ = GetPointOffsetX(origin_pt_, 10); }
  ...
  public:
    ...
    void AdditionalTestSetup() override
    {
        ...
        agent_->Debug()->DebugCreateUnit(
          UNIT_TYPEID::PROTOSS_PYLON, origin_pt_, agent_->Observation()->GetPlayerID(), 1);
        agent_->Debug()->DebugCreateUnit(
          UNIT_TYPEID::PROTOSS_PYLON, GetPointOffsetX(origin_pt_), agent_->Observation()->GetPlayerID(), 1);
        agent_->Debug()->SendDebug();
    }
...
class TestTrainRoach : public TestUnitCommandNoTarget
{
  public:
    ...
    {
        if (ability_command_sent_) { return; }
    ...
    void OnTestFinish() override
    {
        ...
        if (roaches.size() != 3) { ReportError("The number of units trained was not correct."); }
...
class TestTransportBunkerLoad : public TestUnitCommandTargetingUnit
{
  public:
    ...
    void OnStep() override
    {
        ...
        if (!orders_verified_ && ability_command_sent_) { VerifyUnitOrders(target_unit_, ABILITY_ID::MOVE); }
    }
    ...
    void OnTestFinish() override
    {
        ...
        if (test_unit_ && test_unit_->passengers.size() != 1) { ReportError("Unit count in bunker is not 1."); }
        if (test_unit_ && test_unit_->cargo_space_max != 6) { ReportError("Bunker cargo space max is not correct."); }
...
class TestTransportBunkerUnloadAll : public TestUnitCommandNoTarget
{
  public:
    ...
    {
        agent_->Debug()->DebugCreateUnit(
          UNIT_TYPEID::TERRAN_MARAUDER, origin_pt_, agent_->Observation()->GetPlayerID(), 3);
        agent_->Debug()->SendDebug();
    }
    ...
    void OnTestFinish() override
    {
        ...
        if (test_unit_ && test_unit_->passengers.size() != 0) { ReportError("Unit count in bunker is not 0."); }
        if (test_unit_ && test_unit_->cargo_space_max != 6) { ReportError("Bunker cargo space max is not correct."); }
...
UnitCommandTestBot::UnitCommandTestBot() : UnitTestBot()
{
    // Sequences.
...
bool TestUnitCommand(int argc, char **argv)
{
    ...
    if (!coordinator.LoadSettings(argc, argv)) { return false; }
    ...
    while (!bot.IsFinished()) { coordinator.Update(); }
