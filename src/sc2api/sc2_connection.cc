...
static int DataHandler(mg_connection *conn, int /*flags*/, char *data, size_t data_len, void *)
{
    ...
    if (!GetClientData(conn, sc2_connection)) { return 0; }

    SC2APIProtocol::Response *response = new SC2APIProtocol::Response();
    if (!response->ParseFromArray(data, (int)data_len)) { return 1; }
...
{
    sc2::Connection *sc2_connection;
    if (!GetClientData(conn, sc2_connection)) { return; }

    if (sc2_connection->connection_closed_callback_) { sc2_connection->connection_closed_callback_(); }
}

Connection::Connection() : connection_(nullptr), verbose_(false), queue_(), mutex_(), condition_(), has_response_(false)
{}
...
bool Connection::Connect(const std::string &address, int port, bool verbose)
{
    ...
    connection_ = mg_connect_websocket_client(
      address.c_str(), port, 0, ebuff, 100, "/sc2api", nullptr, DataHandler, ConnectionClosedHandler, (void *)this);

    if (!connection_) { return false; }

    if (verbose_) { std::cout << "Connected..." << std::endl; }
...
void Connection::Send(const SC2APIProtocol::Request *request)
{
    ...
    if (verbose_) { std::cout << "Sending: " << request->DebugString(); }
}
...
bool Connection::Receive(SC2APIProtocol::Response *&response, unsigned int timeout_ms)
{
    ...
    if (verbose_) { std::cout << "Waiting for response..." << std::endl; }
...
void Connection::PopResponse(SC2APIProtocol::Response *&response)
{
    ...
    queue_.pop_front();
    if (queue_.empty()) { has_response_ = false; }
}

void Connection::SetTimeoutCallback(std::function<void()> callback) { timeout_callback_ = callback; }

void Connection::SetConnectionClosedCallback(std::function<void()> callback) { connection_closed_callback_ = callback; }

bool Connection::HasConnection() const { return connection_ != nullptr; }
...
bool Connection::PollResponse() { return has_response_; }
