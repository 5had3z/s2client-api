...
bool ParseFromFile(ProcessSettings &process_settings, GameSettings &game_settings, const std::string &file_name)
{
    ...
    if (!reader.LoadFile(file_name)) { return false; }
...
bool ParseSettings(int argc, char *argv[], ProcessSettings &process_settings, GameSettings &game_settings)
{
    ...
    bool parsed = false;
    {
        ...
        if (execute_info_filepath.length() > 0) {
            ...
            if (parsed) { parsed = FindLatestExe(process_settings.process_path); }
        }
    }

    arg_parser.AddOptions({ { "-e", "--executable", "The path to StarCraft II.", true },
    ...
    if (arg_parser.Get("step_size", step_size)) { process_settings.step_size = atoi(step_size.c_str()); }

    std::string realtime;
    if (arg_parser.Get("realtime", realtime)) { process_settings.realtime = realtime == "true" ? true : false; }

    std::string timeout;
    if (arg_parser.Get("timeout", timeout)) { process_settings.timeout_ms = atoi(timeout.c_str()); }
