...
ProtoInterface::ProtoInterface()
  : address_("127.0.0.1"), port_(5000), default_timeout_ms_(kDefaultProtoInterfaceTimeout),
    latest_status_(SC2APIProtocol::Status::unknown), response_pending_(SC2APIProtocol::Response::RESPONSE_NOT_SET)
{}
...
bool ProtoInterface::ConnectToGame(const std::string &address, int port, int timeout_ms)
{
    ...
    if (!connection_.Connect(address, port, false)) { return false; }
...
bool ProtoInterface::SendRequest(GameRequestPtr &request, bool ignore_pending_requests)
{
    ...
    if (request_type >= count_uses_.size()) {
        ...
        for (uint32_t i = current; i < request_type + 1; ++i) { count_uses_[i] = 0; }
    }
    ++count_uses_[request_type];

    // If there is no connection, try rebuilding the connection.
    if (!connection_.HasConnection()) {
        if (!connection_.Connect(address_, port_, false)) { return false; }
    }

    // If there is still no connection, give up.
    if (!connection_.HasConnection()) { return false; }
...
GameResponsePtr ProtoInterface::WaitForResponseInternal()
{
    ...
    if (response) {
        if (response->has_status()) { latest_status_ = response->status(); }
...
bool ProtoInterface::PollResponse() { return connection_.PollResponse(); }
