...
struct Point3D
{
    ...
    Point3D(float in_x, float in_y, float in_z) : x(in_x), y(in_y), z(in_z) {}
...
struct Point2D
{
    ...
    Point2D(float in_x, float in_y) : x(in_x), y(in_y) {}
...
struct Point2DI
{
    ...
    Point2DI(int in_x = 0, int in_y = 0) : x(in_x), y(in_y) {}
...
template<typename Container> typename Container::value_type &GetRandomEntry(Container &container)
{
