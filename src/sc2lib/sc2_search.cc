...
namespace search {
    ...
    size_t CalculateQueries(float radius,
      float step_size,
      ...
      std::vector<QueryInterface::PlacementQuery> &queries)
    {
        ...
        while (loc < 360.0f) {
            Point2D point = Point2D(
              (radius * std::cos((loc * PI) / 180.0f)) + center.x, (radius * std::sin((loc * PI) / 180.0f)) + center.y);
      ...
      ExpansionParameters parameters)
    {
        ...
        for (int i = 0; i < clusters.size(); ++i) {
            ...
            if (parameters.debug_) { parameters.debug_->DebugSphereOut(expansion, 0.35f, Colors::Red); }
