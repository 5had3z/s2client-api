...
template<class Stream> bool IsReading(const Stream &) { return typeid(Stream) == typeid(std::ifstream); }

// Strings.
static inline void SerializeT(std::ifstream &s, std::string &t) { std::getline(s, t); }

static inline void SerializeT(std::ofstream &s, const std::string &t) { s << t << std::endl; }

// Bools.
static inline void SerializeT(std::ifstream &s, bool &t)
{
    std::string linein;
    if (!std::getline(s, linein)) return;
