...
class ActionImp : public ActionInterface
{
  public:
    ...
    void UnitCommand(const Units &unit_tags,
      AbilityID ability,
      const Point2D &point,
      bool queued_command = false) override;
    void
      UnitCommand(const Units &unit_tags, AbilityID ability, const Unit *target, bool queued_command = false) override;
...
{
    if (request_actions_ == nullptr) { request_actions_ = proto_.MakeRequest(); }
    return request_actions_->mutable_action();
}

const std::vector<Tag> &ActionImp::Commands() const { return commands_; }

void ActionImp::SendActions()
{
    commands_.clear();

    if (request_actions_ == nullptr) { return; }

    if (!proto_.SendRequest(request_actions_)) { return; }
    ...
    if (request_action) {
        for (int i = 0, e = request_action->actions_size(); i < e; ++i) {
            ...
            for (auto tag : action.action_raw().unit_command().unit_tags()) { commands_.push_back(tag); };
        }
    }
...
void ActionImp::ToggleAutocast(const std::vector<Tag> &unit_tags, AbilityID ability)
{
    ...
    for (const auto &u : unit_tags) { autocast->add_unit_tags(u); }
...
void ActionImp::SendChat(const std::string &message, ChatChannel channel)
{
    ...
    if (Convert(channel, channel_proto)) { action_chat->set_channel(channel_proto); }
}
...
bool AgentControlImp::Restart()
{
    ...
    if (!control_interface_->Proto().SendRequest(request)) { return false; }
...
Agent::Agent() : agent_control_imp_(nullptr) { agent_control_imp_ = new AgentControlImp(this, Control()); }

Agent::~Agent() { delete agent_control_imp_; }

ActionInterface *Agent::Actions() { return agent_control_imp_->actions_.get(); }

ActionFeatureLayerInterface *Agent::ActionsFeatureLayer() { return agent_control_imp_->actions_feature_layer_.get(); }

AgentControlInterface *Agent::AgentControl() { return agent_control_imp_; }
