...
ProcessSettings::ProcessSettings(bool in_realtime,
  ...
  : realtime(in_realtime), step_size(in_step_size), process_path(in_process_path), net_address(in_net_address),
    timeout_ms(in_timeout_ms), port_start(in_port_start), multi_threaded(in_multi_threaded)
{}

InterfaceSettings::InterfaceSettings() : use_feature_layers(false), use_render(false) {}

GameSettings::GameSettings() : map_name(""), player_setup(), ports() {}

ReplaySettings::ReplaySettings() : replay_file(), replay_dir(), player_id(1) {}
