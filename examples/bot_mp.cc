...
int main(int argc, char *argv[])
{
    ...
    if (PlayerOneIsHuman) { coordinator.SetRealtime(true); }
    ...
    if (PlayerOneIsHuman) { player_one = &human_bot; }
    ...
    bool do_break = false;
    while (!do_break) {
        if (!coordinator.StartGame(sc2::kMapBelShirVestigeLE)) { break; }
        while (coordinator.Update() && !do_break) {
            if (sc2::PollKeyPress()) { do_break = true; }
        }
    }
