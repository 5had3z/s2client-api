...
bool ArgParser::Parse(int argc, char *argv[])
{
    ...
    for (int i = 0; i < argc; ++i) {
        ...
        if (fullname[1] != '-') { fullname = abbv_to_full_[fullname]; }

        std::string value;
        if (i < argc - 1) {
            if (argv[i + 1][0] != '-') { value = argv[i + 1]; }
        }
...
bool ArgParser::Get(const std::string &identifier, std::string &value)
{
    ...
    if (fullname.size() == 1) {
        ...
        if (f == abbv_to_full_.end()) { return false; }

        fullname = f->second;
    }

    if (fullname[0] == '-') { fullname.erase(0, 2); }

    std::unordered_map<std::string, std::string>::const_iterator f = full_to_value_.find(fullname);

    if (f == full_to_value_.end()) { return false; }
