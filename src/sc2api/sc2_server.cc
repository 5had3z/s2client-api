...
static int WebSocketConnectHandler(const mg_connection *conn, void *websocket_server)
{
    ...
    Server *server;
    if (!GetServerData(conn, websocket_server, server)) { return 0; }
...
static int
  WebSocketDataHandler(struct mg_connection *conn, int /*flags*/, char *data, size_t len, void *websocket_server)
{
    Server *server;
    if (!GetServerData(conn, websocket_server, server)) { return 0; }
...
{
    sc2::Server *server;
    if (!GetServerData(conn, websocket_server, server)) { return; }
...
template<class T> static void SendMessage(mg_connection *conn, std::queue<T> &message_queue)
{
...
Server::~Server() { mg_stop(mg_context_); }

bool Server::Listen(const char *listeningPorts,
  ...
  const char *numThreads)
{
    ...
    if (!mg_context_) { return false; }

    mg_set_websocket_handler(mg_context_,
      "/sc2api",
...
const RequestData &Server::PeekRequest() { return requests_.front(); }

const ResponseData &Server::PeekResponse() { return responses_.front(); }
