  ...
  int client_num = 0)
{
    ...
    for (const std::string &command : process_settings.extra_command_lines) cl.push_back(command);
...
static void CallOnStep(Agent *a)
{
    ...
    if (action) { action->SendActions(); }

    ActionFeatureLayerInterface *action_feature_layer = a->ActionsFeatureLayer();
    if (action_feature_layer) { action_feature_layer->SendActions(); }
}
...
CoordinatorImp::CoordinatorImp()
  : agents_(), replay_observers_(), game_ended_(), starcraft_started_(false), game_settings_(),
    process_settings_(false, 1, "", "127.0.0.1", kDefaultProtoInterfaceTimeout, 8168, false)
{}

CoordinatorImp::~CoordinatorImp()
{
    for (auto &p : process_settings_.process_info) { TerminateProcess(p.process_id); }
}

bool CoordinatorImp::AnyObserverAvailable() const
{
    return std::any_of(replay_observers_.cbegin(), replay_observers_.cend(), [](ReplayObserver *r) {
...
bool CoordinatorImp::ShouldRelaunch(ReplayObserver *r)
{
    ...
    if (!FindBaseExe(process_settings_.process_path, replay_info.base_build)) return false;
...
void CoordinatorImp::StartReplay()
{
    ...
    if (replay_settings_.replay_file.empty()) { return; }
    ...
    for (auto r : replay_observers_) {
        ...
        if (!r->Control()->IsReadyForCreateGame()) { continue; }
        ...
        while (replays.size() != 0) {
            ...
            if (ShouldRelaunch(r)) { break; }

            bool launched = r->ReplayControl()->LoadReplay(
              file, interface_settings_, replay_settings_.player_id, process_settings_.realtime);
            replays.pop_back();
            if (launched) break;
        }
    }
...
void CoordinatorImp::StepAgents()
{
    auto step_agent = [this](Agent *a) {
        ...
        control->WaitStep();
        if (process_settings_.multi_threaded) { CallOnStep(a); }
    };
    ...
    if (!process_settings_.multi_threaded) {
        for (auto a : agents_) {
            if (a->Control()->GetAppState() != AppState::normal) { continue; }

            // It is possible to have a pending leave game request here.
            if (a->Control()->PollLeaveGame()) { continue; }
...
void CoordinatorImp::StepAgentsRealtime()
{
    auto step_agent = [](Agent *a) {
        ...
        if (a->Control()->IsFinishedGame()) { return; }
    ...
    if (process_settings_.multi_threaded) {
        ...
        for (auto a : agents_) { step_agent(a); }
    }
}
...
void CoordinatorImp::StepReplayObservers()
{
    ...
    auto run_replay = [this](ReplayObserver *r) {
        if (r->Control()->GetAppState() != AppState::normal) { return; }
        ...
        if (r->Control()->HasResponsePending()) {
            ...
            if (replay_observers_.size() > 1 && !r->Control()->PollResponse()) { return; }
        ...
        if (r->Control()->IsInGame()) {
            ...
            if (!r->Control()->IsInGame()) { r->OnGameEnd(); }
        }
    };
    ...
    if (replay_observers_.size() == 1) {
        ...
        for (auto r : replay_observers_) { threads.emplace_back(run_replay, r); }
    ...
    if (!process_settings_.multi_threaded) {
        for (auto r : replay_observers_) {
            if (r->Control()->GetAppState() != AppState::normal) { continue; }
...
void CoordinatorImp::StepReplayObserversRealtime()
{
    ...
    auto run_replay = [this](ReplayObserver *r) {
        ...
        if (r->Control()->IsInGame()) {
            ...
            // If multithreaded run everyones OnStep in parallel.
            if (process_settings_.multi_threaded) { r->Control()->IssueEvents(); }
...
bool CoordinatorImp::WaitForAllResponses()
{
    ...
    for (;;) {
        ...
        for (Agent *agent : agents_) {
            ...
            if (agent->Control()->PollResponse()) { agent->Control()->ConsumeResponse(); }

            break;
        }

        for (ReplayObserver *replay_observer : replay_observers_) {
            if (has_responses) { break; }
            ...
            if (replay_observer->Control()->PollResponse()) { replay_observer->Control()->ConsumeResponse(); }
...
bool CoordinatorImp::CreateGame()
{
    ...
    return firstClient->Control()->CreateGame(
      game_settings_.map_name, game_settings_.player_setup, process_settings_.realtime);
}
...
bool CoordinatorImp::JoinGame()
{
    ...
    for (auto c : agents_) { c->Control()->WaitJoinGame(); }
    ...
    for (auto c : agents_) { c->Control()->GetObservation(); }
    for (auto c : agents_) { c->OnGameFullStart(); }
    for (auto c : agents_) {
        c->Control()->OnGameStart();
        c->OnGameStart();
    }
    for (auto c : agents_) { c->Control()->IssueEvents(c->Actions()->Commands()); }
...
Coordinator::Coordinator() { imp_ = new CoordinatorImp(); }

Coordinator::~Coordinator() { delete imp_; }

bool Coordinator::StartGame(const std::string &map_path)
{
    if (!map_path.empty()) imp_->game_settings_.map_name = map_path;
    return imp_->StartGame();
}

bool Coordinator::JoinGame() { return imp_->JoinGame(); }
...
bool Coordinator::RemoteSaveMap(const void *data, int data_size, std::string remote_path)
{
    for (auto c : imp_->agents_) {
        if (!c->Control()->RemoteSaveMap(data, data_size, remote_path)) return false;
    }
...
void Coordinator::SetParticipants(const std::vector<PlayerSetup> &participants)
{
    ...
    for (const auto &p : participants) {
        if (p.agent) { imp_->AddAgent(p.agent); }
        imp_->game_settings_.player_setup.push_back(p);
    }
}

void Coordinator::SetReplayRecovery(bool value) { imp_->replay_recovery_ = value; }
...
void Coordinator::Connect(int port)
{
    while (imp_->process_settings_.process_info.size() < imp_->agents_.size()) {
        imp_->process_settings_.process_info.push_back(ProcessInfo(imp_->process_settings_.net_address, 0, port));
    }
...
{
    for (auto c : imp_->agents_) { c->Control()->RequestLeaveGame(); }
}

void Coordinator::WaitForAllResponses() { imp_->WaitForAllResponses(); }
...
bool Coordinator::Update()
{
    ...
    if (imp_->replay_observers_.size() > 0) {
        if (imp_->AnyObserverAvailable()) { imp_->StartReplay(); }
    }
...
bool Coordinator::AllGamesEnded() const
{
    for (auto a : imp_->agents_) {
        if (a->Control()->IsInGame() || a->Control()->HasResponsePending()) { return false; }
    }

    for (auto r : imp_->replay_observers_) {
        if (r->Control()->IsInGame() || r->Control()->HasResponsePending()) { return false; }
    }
...
void Coordinator::SetMultithreaded(bool value) { imp_->process_settings_.multi_threaded = value; }
...
void Coordinator::SetTimeoutMS(uint32_t timeout_ms) { imp_->process_settings_.timeout_ms = timeout_ms; }
...
void Coordinator::SetReplayPerspective(uint32_t perspective) { imp_->replay_settings_.player_id = perspective; }

uint32_t Coordinator::GetReplayPerspective() const { return imp_->replay_settings_.player_id; }
...
{
    if (!DoesFileExist(file_path)) return false;
...
void Coordinator::SaveReplayList(const std::string &file_path)
{
    ...
    for (const std::string &line : imp_->replay_settings_.replay_file) { replay_file << line << std::endl; }
}

bool Coordinator::HasReplays() const { return !imp_->replay_settings_.replay_file.empty(); }
...
{
    if (imp_->process_settings_.process_path.length() > 4) return imp_->process_settings_.process_path;
...
void Coordinator::SetupPorts(size_t num_agents, int port_start, bool check_single)
{
    ...
    if (check_single) {
        for (const auto &p_setup : imp_->game_settings_.player_setup) {
            if (p_setup.type == sc2::PlayerType::Participant) { ++humans; }
        }
    } else {
        humans = num_agents;
    }
