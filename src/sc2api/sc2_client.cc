...
class ObservationImp : public ObservationInterface
{
  public:
    ...
    ObservationImp(ProtoInterface &proto,
...
ObservationImp::ObservationImp(ProtoInterface &proto,
  ObservationPtr &observation,
  ResponseObservationPtr &response,
  ControlInterface &control)
  : proto_(proto), observation_(observation), response_(response), control_(control),
    current_game_loop_(std::numeric_limits<uint32_t>::max()), previous_game_loop(std::numeric_limits<uint32_t>::max())
{
    ClearFlags();
}
...
const Unit *ObservationImp::GetUnit(Tag tag) const { return unit_pool_.GetExistingUnit(tag); }

Units ObservationImp::GetUnits(Unit::Alliance alliance, Filter filter) const
{
    Units units;
    unit_pool_.ForEachExistingUnit([&](Unit &unit) {
        if (unit.alliance != alliance) { return; }

        if (!filter || filter(unit)) { units.push_back(&unit); }
    });
    return units;
}
...
{
    if (force_refresh || abilities_.size() < 1) { abilities_cached_ = false; }

    if (abilities_cached_) { return abilities_; }
    ...
    if (!proto_.SendRequest(request)) { return abilities_; }
    ...
    if (response_data.HasErrors()) { return abilities_; }

    if (response_data.HasErrors() || response_data->abilities_size() == 0) { return abilities_; }
...
{
    if (force_refresh || unit_types_.size() < 1) { unit_types_cached = false; }

    if (unit_types_cached) { return unit_types_; }
    ...
    if (!proto_.SendRequest(request)) { return unit_types_; }

    GameResponsePtr response = control_.WaitForResponse();
    ResponseDataPtr response_data;
    SET_MESSAGE_RESPONSE(response_data, response, data);
    if (response_data.HasErrors()) { return unit_types_; }

    if (response_data.HasErrors() || response_data->units_size() == 0) { return unit_types_; }
...
{
    if (force_refresh || upgrade_ids_.size() < 1) { upgrades_cached_ = false; }

    if (upgrades_cached_) { return upgrade_ids_; }
    ...
    if (!proto_.SendRequest(request)) { return upgrade_ids_; }

    GameResponsePtr response = control_.WaitForResponse();
    ResponseDataPtr response_data;
    SET_MESSAGE_RESPONSE(response_data, response, data);
    if (response_data.HasErrors()) { return upgrade_ids_; }

    if (response_data.HasErrors() || response_data->upgrades_size() == 0) { return upgrade_ids_; }
...
{
    if (force_refresh || buff_ids_.size() < 1) { buffs_cached_ = false; }

    if (buffs_cached_) { return buff_ids_; }
    ...
    if (!proto_.SendRequest(request)) { return buff_ids_; }

    GameResponsePtr response = control_.WaitForResponse();
    ResponseDataPtr response_data;
    SET_MESSAGE_RESPONSE(response_data, response, data);
    if (response_data.HasErrors()) { return buff_ids_; }

    if (response_data.HasErrors() || response_data->buffs_size() == 0) { return buff_ids_; }
...
{
    if (force_refresh || effect_ids_.size() < 1) { effects_cached_ = false; }

    if (effects_cached_) { return effect_ids_; }
    ...
    if (!proto_.SendRequest(request)) { return effect_ids_; }

    GameResponsePtr response = control_.WaitForResponse();
    ResponseDataPtr response_data;
    SET_MESSAGE_RESPONSE(response_data, response, data);
    if (response_data.HasErrors()) { return effect_ids_; }

    if (response_data.HasErrors() || response_data->effects_size() == 0) { return effect_ids_; }

    effect_ids_.resize(response_data->effects_size());
    for (int i = 0; i < response_data->effects_size(); ++i) { effect_ids_[i].ReadFromProto(response_data->effects(i)); }
...
{
    if (game_info_cached_) { return game_info_; }

    GameRequestPtr request = proto_.MakeRequest();
    request->mutable_game_info();

    if (!proto_.SendRequest(request)) { return game_info_; }

    GameResponsePtr response = control_.WaitForResponse();
    ResponseGameInfoPtr response_game_info;
    SET_MESSAGE_RESPONSE(response_game_info, response, game_info);
    if (response_game_info.HasErrors()) { return game_info_; }
...
static bool SampleImageData(const std::string &data, int width, int height, const Point2D &point, unsigned char &result)
{
    ...
    if (pointI.x < 0 || pointI.x >= width || pointI.y < 0 || pointI.y >= height) { return false; }
...
Visibility ObservationImp::GetVisibility(const Point2D &point) const
{
    ObservationRawPtr observation_raw;
    SET_SUBMESSAGE_RESPONSE(observation_raw, observation_, raw_data);
    if (observation_raw.HasErrors()) { return Visibility::FullHidden; }

    const SC2APIProtocol::MapState &map_state = observation_raw->map_state();
    const SC2APIProtocol::ImageData &visibility = map_state.visibility();

    unsigned char value;
    if (!SampleImageData(visibility, point, value)) return Visibility::FullHidden;
...
bool ObservationImp::IsPathable(const Point2D &point) const
{
    ...
    unsigned char value;
    if (!SampleImageData(game_info.pathing_grid, point, value)) return false;
...
bool ObservationImp::IsPlacable(const Point2D &point) const
{
    ...
    unsigned char value;
    if (!SampleImageData(game_info.placement_grid, point, value)) return false;
...
float ObservationImp::TerrainHeight(const Point2D &point) const
{
    ...
    unsigned char value;
    if (!SampleImageData(game_info.terrain_height, point, value)) return false;
...
bool ObservationImp::UpdateObservation()
{
    ...
    if (!Convert(observation_, score_)) { return false; }
    ...
    if (player_common.has_player_id()) { player_id_ = player_common.player_id(); }
    ...
    chat_.clear();
    for (auto &message : response_->chat()) { chat_.push_back({ message.player_id(), message.message() }); }
    ...
    unit_pool_.ForEachExistingUnit([&](Unit &unit) { units_previous_map_[unit.tag] = unit; });
    ...
    if (!observation_raw->has_player()) { return false; }

    const SC2APIProtocol::PlayerRaw &player_raw = observation_raw->player();
    if (!player_raw.has_camera()) { return false; }
    ...
    for (int i = 0, e = player_raw.power_sources_size(); i < e; ++i) {
        ...
        power_sources_.push_back(PowerSource(
          Point2D(power_source.pos().x(), power_source.pos().y()), power_source.radius(), power_source.tag()));
    }

    upgrades_previous_ = upgrades_;
    upgrades_.clear();
    for (int i = 0; i < player_raw.upgrade_ids_size(); ++i) { upgrades_.push_back(player_raw.upgrade_ids(i)); }

    player_results_.clear();
    for (const auto &player_result : response_->player_result()) {
        player_results_.push_back(
          PlayerResult(player_result.player_id(), ConvertGameResultFromProto(player_result.result())));
    }

    return true;
}

const SC2APIProtocol::Observation *ObservationImp::GetRawObservation() const { return observation_.get(); }
...
AvailableAbilities QueryImp::GetAbilitiesForUnit(const Unit *unit, bool ignore_resource_requirements)
{
    ...
    if (available_abilities.size() < 1) { return AvailableAbilities(); }
...
std::vector<AvailableAbilities> QueryImp::GetAbilitiesForUnits(const Units &units, bool ignore_resource_requirements)
{
    ...
    // Make the request.
    {
        if (units.empty()) { return available_abilities_out; }
        ...
        if (!proto_.SendRequest(request)) { return available_abilities_out; }
    }

    // Process the response.
    GameResponsePtr response = control_.WaitForResponse();
    if (!response.get()) { return available_abilities_out; }
    ...
    if (query.abilities_size() < 1) { return available_abilities_out; }
...
std::vector<float> QueryImp::PathingDistance(const std::vector<PathingQuery> &queries)
{
    ...
    if (!proto_.SendRequest(request)) { return std::vector<float>(queries.size(), 0.0f); }

    GameResponsePtr response = control_.WaitForResponse();
    ResponseQueryPtr response_query;
    SET_MESSAGE_RESPONSE(response_query, response, query);
    if (response_query.HasErrors()) { return std::vector<float>(queries.size(), 0.0f); }

    if (response_query->pathing_size() != queries.size()) { return std::vector<float>(queries.size(), 0.0f); }
...
std::vector<bool> QueryImp::Placement(const std::vector<PlacementQuery> &queries)
{
    ...
    if (!proto_.SendRequest(request)) { return std::vector<bool>(queries.size(), false); }

    GameResponsePtr response = control_.WaitForResponse();
    ResponseQueryPtr response_query;
    SET_MESSAGE_RESPONSE(response_query, response, query);
    if (response_query.HasErrors()) { return std::vector<bool>(queries.size(), false); }

    if (response_query->placements_size() != queries.size()) { return std::vector<bool>(queries.size(), false); }
...
class DebugImp : public DebugInterface
{
  public:
    ...
    void DebugTextOut(const std::string &out,
      const Point2D &pt_virtual_2D,
      Color color = Colors::White,
      uint32_t size = 8) override;
    void DebugTextOut(const std::string &out,
      const Point3D &pt3D,
      Color color = Colors::White,
      uint32_t size = 8) override;
...
DebugImp::DebugImp(ProtoInterface &proto, ObservationInterface &observation, ControlInterface &control)
  : proto_(proto), observation_(observation), control_(control), has_move_camera(false), app_test_set_(false),
    endgame_surrender_(false), endgame_victory_(false), set_score_(false), score_(0.0f)
{}
...
void DebugImp::DebugShowMap() { debug_state_.push_back(SC2APIProtocol::DebugGameState::show_map); }

void DebugImp::DebugEnemyControl() { debug_state_.push_back(SC2APIProtocol::DebugGameState::control_enemy); }

void DebugImp::DebugIgnoreFood() { debug_state_.push_back(SC2APIProtocol::DebugGameState::food); }

void DebugImp::DebugIgnoreResourceCost() { debug_state_.push_back(SC2APIProtocol::DebugGameState::free); }

void DebugImp::DebugGiveAllResources() { debug_state_.push_back(SC2APIProtocol::DebugGameState::all_resources); }

void DebugImp::DebugGodMode() { debug_state_.push_back(SC2APIProtocol::DebugGameState::god); }

void DebugImp::DebugIgnoreMineral() { debug_state_.push_back(SC2APIProtocol::DebugGameState::minerals); }

void DebugImp::DebugNoCooldowns() { debug_state_.push_back(SC2APIProtocol::DebugGameState::cooldown); }

void DebugImp::DebugGiveAllTech() { debug_state_.push_back(SC2APIProtocol::DebugGameState::tech_tree); }

void DebugImp::DebugGiveAllUpgrades() { debug_state_.push_back(SC2APIProtocol::DebugGameState::upgrade); }

void DebugImp::DebugFastBuild() { debug_state_.push_back(SC2APIProtocol::DebugGameState::fast_build); }
...
void DebugImp::SendDebug()
{
    ...
    if (debug_kill_tag_.size() > 0) {
        ...
        for (std::size_t i = 0; i < debug_kill_tag_.size(); ++i) { debug_kill_unit->add_tag(debug_kill_tag_[i]); }
    }
...
ControlImp::ControlImp(Client &client)
  : client_(client), app_state_(AppState::normal), is_multiplayer_(false), observation_imp_(nullptr),
    query_imp_(nullptr), debug_imp_(nullptr)
{
...
ControlImp::~ControlImp() { proto_.Quit(); }

ProtoInterface &ControlImp::Proto() { return proto_; }
...
{
    if (!is_multiplayer_) { return false; }
...
{
    GameResponsePtr response = WaitForResponse();
    if (!response.get() || !response->has_step() || response->error_size() > 0) { return false; }
...
bool ControlImp::SaveReplay(const std::string &path)
{
    ...
    GameResponsePtr response = WaitForResponse();
    if (!response.get() || !response->has_save_replay() || response->error_size() > 0) { return false; }

    const SC2APIProtocol::ResponseSaveReplay &response_replay = response->save_replay();

    if (response_replay.data().size() == 0) { return false; }
...
bool ControlImp::Ping() { return proto_.PingGame(); }
...
GameResponsePtr ControlImp::WaitForResponse()
{
    ...
    if (response.get() && response->error_size() > 0) {
        ...
        for (int i = 0; i < response->error_size(); ++i) { errors.push_back(response->error(i)); }
    ...
    // Step 2: distinguish between a non-responsive app and a failure to deliver a valid request.
    {
        ...
        if (!proto_.SendRequest(ping_request, true)) {
            ...
            if (response_ping) {
                if (proto_.GetLastStatus() == SC2APIProtocol::Status::unknown) { Error(ClientError::SC2UnknownStatus); }
...
void ControlImp::SetProcessInfo(const ProcessInfo &pi) { pi_ = pi; }

const ProcessInfo &ControlImp::GetProcessInfo() const { return pi_; }

SC2APIProtocol::Status ControlImp::GetLastStatus() const { return proto_.GetLastStatus(); }

AppState ControlImp::GetAppState() const { return app_state_; }
...
{
    if (app_state_ != AppState::normal) return true;
...
bool ControlImp::HasResponsePending() const { return proto_.HasResponsePending(); }
...
bool ControlImp::GetObservation()
{
    ...
    if (observation.HasErrors()) { return false; }
...
bool ControlImp::PollResponse() { return proto_.PollResponse(); }
...
{
    if (!observation_->has_raw_data()) { return; }
...
void ControlImp::IssueUnitAddedEvents()
{
    observation_imp_->unit_pool_.ForEachExistingUnit([&](sc2::Unit &unit) {
        ...
        if (found != observation_imp_->units_previous_map_.end()) { return; }
...
{
    if (!unit || !unit->orders.empty() || unit->build_progress < 1.0f) { return; }
...
void ControlImp::IssueBuildingCompletedEvent(const Unit *unit)
{
    ...
    if (!unit || unit->build_progress < 1.0f) { return; }
    ...
    const Unit &unit_previous = found->second;
    if (unit_previous.build_progress < 1.0f) { client_.OnBuildingConstructionComplete(unit); }
}
...
void ControlImp::IssueUpgradeEvents()
{
    ...
    for (auto up : observation_imp_->upgrades_previous_) { previous.insert(up); }

    for (auto up : observation_imp_->upgrades_) {
        if (previous.find(up) == previous.end()) { client_.OnUpgradeCompleted(up); }
    }
}

bool ControlImp::IssueEvents(const std::vector<Tag> &commands)
{
    if (observation_imp_->current_game_loop_ == observation_imp_->previous_game_loop) { return false; }
...
void ControlImp::Error(ClientError error, const std::vector<std::string> &errors)
{
    ...
    for (auto const &e : errors) { protocol_errors_.push_back(e); }
...
{
    if (condition) { Error(error, errors); }
}
...
void ControlImp::Save()
{
    ...
    if (!Proto().SendRequest(request)) { return; }
    WaitForResponse();
}
...
Client::Client() : control_imp_(nullptr) { control_imp_ = new ControlImp(*this); }

Client::~Client() { delete control_imp_; }
...
DebugInterface *Client::Debug() { return control_imp_->debug_imp_.get(); }

ControlInterface *Client::Control() { return control_imp_; }

const ControlInterface *Client::Control() const { return control_imp_; }
...
bool IsVisible::operator()(const Unit &unit) { return unit.display_type == Unit::Visible; };
