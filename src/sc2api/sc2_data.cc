...
AbilityData::AbilityData()
  : available(false), ability_id(0), link_index(0), remaps_to_ability_id(0), target(Target::None), allow_minimap(false),
    allow_autocast(false), is_building(false), footprint_radius(0.0f), is_instant_placement(false), cast_range(0.0f)
{}

void AbilityData::ReadFromProto(const SC2APIProtocol::AbilityData &ability_data)
{
    // ability_id_
    if (ability_data.has_ability_id()) { ability_id = ability_data.ability_id(); }

    // available_
    available = false;
    if (ability_data.has_available()) { available = ability_data.available(); }

    // link_name_
    if (ability_data.has_link_name()) { link_name = ability_data.link_name(); }

    // link_index_
    link_index = 0;
    if (ability_data.has_link_index()) { link_index = ability_data.link_index(); }

    // button_name_
    if (ability_data.has_button_name()) { button_name = ability_data.button_name(); }

    // friendly_name_
    if (ability_data.has_friendly_name()) { friendly_name = ability_data.friendly_name(); }

    // hotkey_
    if (ability_data.has_hotkey()) { hotkey = ability_data.hotkey(); }

    // Remaps.
    remaps_to_ability_id = 0;
    if (ability_data.has_remaps_to_ability_id()) { remaps_to_ability_id = ability_data.remaps_to_ability_id(); }
    ...
    // allow_minimap_
    allow_minimap = 0;
    if (ability_data.has_allow_minimap()) { allow_minimap = ability_data.allow_minimap(); }

    // allow_minimap_
    allow_autocast = 0;
    if (ability_data.has_allow_autocast()) { allow_autocast = ability_data.allow_autocast(); }

    // is_building_
    is_building = false;
    if (ability_data.has_is_building()) { is_building = ability_data.is_building(); }

    // footprint_radius_
    footprint_radius = 0.0f;
    if (ability_data.has_footprint_radius()) { footprint_radius = ability_data.footprint_radius(); }

    is_instant_placement = false;
    if (ability_data.has_is_instant_placement()) { is_instant_placement = ability_data.has_is_instant_placement(); }

    cast_range = 0.0f;
    if (ability_data.has_cast_range()) { cast_range = ability_data.cast_range(); }
}
...
std::string AbilityData::Log() const
{
    ...
    if (button_name.length() > 0) { str_out += "  Button: " + button_name + "\n"; }
    if (hotkey.length() > 0) { str_out += "  Hotkey: " + hotkey + "\n"; }
    str_out += "  " + (link_name.length() > 0 ? link_name : "Null") + ", " + std::to_string(link_index) + "\n";
    if (is_building) { str_out += "  Building footprint: " + std::to_string(footprint_radius) + "\n"; }
...
void UnitTypeData::ReadFromProto(const SC2APIProtocol::UnitTypeData &unit_data)
{
    ...
    for (int i = 0; i < unit_data.tech_alias_size(); ++i) { tech_alias.push_back(unit_data.tech_alias(i)); }
...
{
    if (ability_id == 0) { return AbilityID(ability_id); }
    ...
    if (ability.remaps_to_ability_id != 0) { return AbilityID(ability.remaps_to_ability_id); }
