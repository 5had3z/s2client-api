...
struct IsFlying
{
    bool operator()(const Unit &unit) { return unit.is_flying; }
};
...
struct IsArmy
{
    ...
    bool operator()(const Unit &unit)
    {
        ...
        for (const auto &attribute : attributes) {
            if (attribute == Attribute::Structure) { return false; }
        }
        switch (unit.unit_type.ToType()) {
        case UNIT_TYPEID::ZERG_OVERLORD:
            return false;
...
struct IsStructure
{
    ...
    bool operator()(const Unit &unit)
    {
        ...
        for (const auto &attribute : attributes) {
            if (attribute == Attribute::Structure) { is_structure = true; }
        }
        return is_structure;
    }
...
int CountUnitType(const ObservationInterface *observation, UnitTypeID unit_type)
{
    ...
    for (const auto unit : my_units) {
        if (unit->unit_type == unit_type) ++count;
    }
...
bool FindEnemyStructure(const ObservationInterface *observation, const Unit *&enemy_unit)
{
    ...
    for (const auto unit : my_units) {
        if (unit->unit_type == UNIT_TYPEID::TERRAN_COMMANDCENTER || unit->unit_type == UNIT_TYPEID::TERRAN_SUPPLYDEPOT
...
size_t MultiplayerBot::CountUnitTypeBuilding(const ObservationInterface *observation,
  UNIT_TYPEID production_building,
  ABILITY_ID ability)
{
    ...
    for (const auto &building : buildings) {
        if (building->orders.empty()) { continue; }

        for (const auto order : building->orders) {
            if (order.ability_id == ability) { building_count++; }
        }
    }
...
size_t MultiplayerBot::CountUnitTypeTotal(const ObservationInterface *observation,
  UNIT_TYPEID unit_type,
  UNIT_TYPEID production,
  ABILITY_ID ability)
{
...
size_t MultiplayerBot::CountUnitTypeTotal(const ObservationInterface *observation,
  std::vector<UNIT_TYPEID> unit_type,
  UNIT_TYPEID production,
  ABILITY_ID ability)
{
    size_t count = 0;
    for (const auto &type : unit_type) { count += CountUnitType(observation, type); }
...
const Unit *MultiplayerBot::FindNearestMineralPatch(const Point2D &start)
{
    ...
    if (distance == std::numeric_limits<float>::max()) { return target; }
    return target;
}
...
void MultiplayerBot::AttackWithUnitType(UnitTypeID unit_type, const ObservationInterface *observation)
{
    ...
    for (const auto &unit : units) { AttackWithUnit(unit, observation); }
}
...
{
    Units units = observation->GetUnits(Unit::Alliance::Self, IsUnit(unit_type));
    for (const auto &unit : units) { ScoutWithUnit(unit, observation); }
}
...
bool MultiplayerBot::TryBuildStructure(AbilityID ability_type_for_structure,
  ...
  bool isExpansion = false)
{
    ...
    if (Query()->PathingDistance(unit, location) < 0.1f) { return false; }
    if (!isExpansion) {
        for (const auto &expansion : expansions_) {
            if (Distance2D(location, Point2D(expansion.x, expansion.y)) < 7) { return false; }
        }
    }
...
bool MultiplayerBot::TryExpand(AbilityID build_ability, UnitTypeID worker_type)
{
    ...
    for (const auto &expansion : expansions_) {
        ...
        if (current_distance < .01f) { continue; }
...
bool MultiplayerBot::TryBuildGas(AbilityID build_ability, UnitTypeID worker_type, Point2D base_location)
{
    ...
    if (closestGeyser == 0) { return false; }
...
bool MultiplayerBot::TryBuildUnit(AbilityID ability_type_for_unit, UnitTypeID unit_type)
{
    ...
    if (!GetRandomUnit(unit, observation, unit_type)) { return false; }
...
void MultiplayerBot::MineIdleWorkers(const Unit *worker,
  AbilityID worker_gather_command,
...
int MultiplayerBot::GetExpectedWorkers(UNIT_TYPEID vespene_building_type)
{
    ...
    for (const auto &base : bases) {
        if (base->build_progress != 1) { continue; }
        expected_workers += base->ideal_harvesters;
    }

    for (const auto &geyser : geysers) {
        if (geyser->vespene_contents > 0) {
            if (geyser->build_progress != 1) { continue; }
...
void MultiplayerBot::ManageWorkers(UNIT_TYPEID worker_type,
  AbilityID worker_gather_command,
  UNIT_TYPEID vespene_building_type)
{
    ...
    Units workers = observation->GetUnits(Unit::Alliance::Self, IsUnit(worker_type));
    for (const auto &geyser : geysers) {
        if (geyser->ideal_harvesters == 0 || geyser->build_progress != 1) { continue; }
        if (geyser->assigned_harvesters > geyser->ideal_harvesters) {
            for (const auto &worker : workers) {
                if (!worker->orders.empty()) {
...
void MultiplayerBot::RetreatWithUnits(UnitTypeID unit_type, Point2D retreat_position)
{
    const ObservationInterface *observation = Observation();
    Units units = observation->GetUnits(Unit::Alliance::Self, IsUnit(unit_type));
    for (const auto &unit : units) { RetreatWithUnit(unit, retreat_position); }
}
...
void ProtossMultiplayerBot::ManageArmy()
{
    ...
    if (enemy_units.empty() && observation->GetFoodArmy() < wait_til_supply) {
        for (const auto &unit : army) { RetreatWithUnit(unit, staging_location_); }
    } else if (!enemy_units.empty()) {
        ...
        for (const auto &unit : army) { ScoutWithUnit(unit, observation); }
    }
}
...
{
    const ObservationInterface *observation = Observation();
    if (observation->GetFoodWorkers() <= target_worker_count_) { return false; }
    // Until we have 2 bases, hold off on building too many units
    if (CountUnitType(observation, UNIT_TYPEID::PROTOSS_NEXUS) < 2 && observation->GetFoodArmy() > 10) { return false; }

    // If we have a decent army already, try hold until we expand again
    if (CountUnitType(observation, UNIT_TYPEID::PROTOSS_NEXUS) < 3 && observation->GetFoodArmy() > 40) { return false; }
    ...
    if (templar.size() > 1) {
        Units templar_merge;
        for (int i = 0; i < 2; ++i) { templar_merge.push_back(templar.at(i)); }
    ...
    if (air_build_) {
        ...
        if (CountUnitType(observation, UNIT_TYPEID::PROTOSS_FLEETBEACON) > 0 && carrier_count < max_colossus_count_) {
            ...
            if (CountUnitType(observation, UNIT_TYPEID::PROTOSS_MOTHERSHIP) < 1 && mothership_count > 0) {
                if (TryBuildUnit(ABILITY_ID::MORPH_MOTHERSHIP, UNIT_TYPEID::PROTOSS_MOTHERSHIPCORE)) { return true; }
                return false;
            }

            if (observation->GetMinerals() > 350 && observation->GetVespene() > 250) {
                if (TryBuildUnit(ABILITY_ID::TRAIN_CARRIER, UNIT_TYPEID::PROTOSS_STARGATE)) { return true; }
        ...
        if (CountUnitType(observation, UNIT_TYPEID::PROTOSS_ROBOTICSBAY) > 0 && colossus_count < max_colossus_count_) {
            if (observation->GetMinerals() > 300 && observation->GetVespene() > 200) {
                if (TryBuildUnit(ABILITY_ID::TRAIN_COLOSSUS, UNIT_TYPEID::PROTOSS_ROBOTICSFACILITY)) { return true; }
            } else if (CountUnitTypeTotal(observation,
            ...
            if (observation->GetMinerals() > 250 && observation->GetVespene() > 100) {
                if (TryBuildUnit(ABILITY_ID::TRAIN_IMMORTAL, UNIT_TYPEID::PROTOSS_ROBOTICSFACILITY)) { return true; }
            }
        }
    }
    ...
    if (warpgate_reasearched_ && CountUnitType(observation, UNIT_TYPEID::PROTOSS_WARPGATE) > 0) {
        ...
        if (CountUnitType(observation, UNIT_TYPEID::PROTOSS_STALKER) > max_stalker_count_) { return false; }

        if (CountUnitType(observation, UNIT_TYPEID::PROTOSS_ADEPT) > max_stalker_count_) { return false; }
        ...
        if (!air_build_) {
            ...
            if (CountUnitType(observation, UNIT_TYPEID::PROTOSS_HIGHTEMPLAR) < 2
                && CountUnitType(observation, UNIT_TYPEID::PROTOSS_ARCHON) < 2) {
...
void ProtossMultiplayerBot::BuildOrder()
{
    ...
    size_t gateway_count = CountUnitType(observation, UNIT_TYPEID::PROTOSS_GATEWAY)
                           + CountUnitType(observation, UNIT_TYPEID::PROTOSS_WARPGATE);
    ...
    if (gateway_count < std::min<size_t>(2 * base_count, 7)) {
        ...
        if (cybernetics_count < 1 && gateway_count > 0) {
            ...
            if (observation->GetFoodWorkers() >= target_worker_count_
                && observation->GetMinerals() > 150 + (100 * gateway_count)) {
...
bool ProtossMultiplayerBot::TryWarpInUnit(ABILITY_ID ability_type_for_unit)
{
    ...
    if (power_sources.empty()) { return false; }
    ...
    Point2D build_location =
      Point2D(random_power_source.position.x + rx * radius, random_power_source.position.y + ry * radius);
    ...
    if (Query()->PathingDistance(build_location, Point2D(game_info_.playable_max.x / 2, game_info_.playable_max.y / 2))
        < .01f) {
        return false;
    }
...
void ProtossMultiplayerBot::ConvertGateWayToWarpGate()
{
    ...
    if (warpgate_reasearched_) {
        for (const auto &gateway : gateways) {
            if (gateway->build_progress == 1) { Actions()->UnitCommand(gateway, ABILITY_ID::MORPH_WARPGATE); }
        }
    }
}
...
bool ProtossMultiplayerBot::TryBuildPylon()
{
    ...
    if (observation->GetFoodUsed() < observation->GetFoodCap() - 6) { return false; }
    ...
    for (const auto &base : bases) {
        if (base->assigned_harvesters >= base->ideal_harvesters) {
            if (base->build_progress == 1) {
                if (TryBuildGas(ABILITY_ID::BUILD_ASSIMILATOR, UNIT_TYPEID::PROTOSS_PROBE, base->pos)) { return true; }
            }
        }
    }
    return false;
...
bool ProtossMultiplayerBot::TryBuildExpansionNexus()
{
    ...
    if (GetExpectedWorkers(UNIT_TYPEID::PROTOSS_ASSIMILATOR) > max_worker_count_) { return false; }
...
bool ProtossMultiplayerBot::TryBuildProbe()
{
    const ObservationInterface *observation = Observation();
    Units bases = observation->GetUnits(Unit::Alliance::Self, IsTownHall());
    if (observation->GetFoodWorkers() >= max_worker_count_) { return false; }

    if (observation->GetFoodUsed() >= observation->GetFoodCap()) { return false; }

    if (observation->GetFoodWorkers() > GetExpectedWorkers(UNIT_TYPEID::PROTOSS_ASSIMILATOR)) { return false; }
...
void ProtossMultiplayerBot::OnStep()
{
    ...
    if (observation->GetFoodUsed() >= observation->GetFoodCap()) { frames_to_skip = 6; }

    if (observation->GetGameLoop() % frames_to_skip != 0) { return; }

    if (!nuke_detected) {
        ManageArmy();
    } else if (nuke_detected) {
        if (nuke_detected_frame + 400 < observation->GetGameLoop()) { nuke_detected = false; }
        Units units = observation->GetUnits(Unit::Self, IsArmy(observation));
        for (const auto &unit : units) { RetreatWithUnit(unit, startLocation_); }
    }
    ...
    if (TryBuildPylon()) { return; }

    if (TryBuildAssimilator()) { return; }

    if (TryBuildProbe()) { return; }

    if (TryBuildArmy()) { return; }

    if (TryBuildExpansionNexus()) { return; }
}
...
void ProtossMultiplayerBot::OnUnitIdle(const Unit *unit)
{
    ...
    case UNIT_TYPEID::PROTOSS_CYBERNETICSCORE: {
        ...
        if (!warpgate_reasearched_) {
            ...
            if (!nexus.empty()) { Actions()->UnitCommand(nexus.front(), ABILITY_ID::EFFECT_TIMEWARP, unit); }
        }
    }
    default:
        break;
...
bool ZergMultiplayerBot::TryBuildDrone()
{
    ...
    for (const auto &egg : eggs) {
        if (!egg->orders.empty()) {
            if (egg->orders.front().ability_id == ABILITY_ID::TRAIN_DRONE) { worker_count++; }
        }
    }
    if (worker_count >= max_worker_count_) { return false; }

    if (worker_count > GetExpectedWorkers(UNIT_TYPEID::ZERG_EXTRACTOR)) { return false; }
...
void ZergMultiplayerBot::BuildOrder()
{
    ...
    for (const auto &hatchery : hatcherys) {
        if (!hatchery->orders.empty()) {
            if (hatchery->orders.front().ability_id == ABILITY_ID::MORPH_LAIR) { ++morphing_lair; }
        }
    }
    for (const auto &lair : lairs) {
        if (!lair->orders.empty()) {
            if (lair->orders.front().ability_id == ABILITY_ID::MORPH_HIVE) { ++morphing_hive; }
        }
    }

    if (!mutalisk_build_) { evolution_chanber_target++; }
    ...
    if (CountUnitType(observation, UNIT_TYPEID::ZERG_SPAWNINGPOOL) < 1) {
        ...
        if (!lair_tech) {
            if (CountUnitType(observation, UNIT_TYPEID::ZERG_HIVE) + CountUnitType(observation, UNIT_TYPEID::ZERG_LAIR)
                  < 1
                && CountUnitType(observation, UNIT_TYPEID::ZERG_QUEEN) > 0) {
                TryBuildUnit(ABILITY_ID::MORPH_LAIR, UNIT_TYPEID::ZERG_HATCHERY);
            }
        } else {
            if (!mutalisk_build_) {
                if (CountUnitType(observation, UNIT_TYPEID::ZERG_HYDRALISKDEN)
                      + CountUnitType(observation, UNIT_TYPEID::ZERG_LURKERDENMP)
                    < 1) {
                ...
                if (CountUnitType(observation, UNIT_TYPEID::ZERG_SPIRE)
                      + CountUnitType(observation, UNIT_TYPEID::ZERG_GREATERSPIRE)
                    < 1) {
            ...
            if (CountUnitType(observation, UNIT_TYPEID::ZERG_INFESTATIONPIT) > 0
                && CountUnitType(observation, UNIT_TYPEID::ZERG_HIVE) < 1) {
    ...
    if (enemy_units.empty() && observation->GetFoodArmy() < wait_til_supply) {
        ...
        for (const auto &unit : army) {
            ...
            case (UNIT_TYPEID::ZERG_CORRUPTOR): {
                ...
                if (!unit->orders.empty()) {
                    if (unit->orders.front().ability_id == ABILITY_ID::EFFECT_CAUSTICSPRAY) { break; }
                }
                AttackWithUnit(unit, observation);
                break;
            }
            ...
            case (UNIT_TYPEID::ZERG_LURKERMPBURROWED): {
                ...
                if (distance > 9) { Actions()->UnitCommand(unit, ABILITY_ID::BURROWUP); }
                break;
            }
            case (UNIT_TYPEID::ZERG_SWARMHOSTMP): {
                ...
                if (distance < 15) {
                    ...
                    for (const auto &ability : abilities) {
                        if (ability.ability_id == ABILITY_ID::EFFECT_SPAWNLOCUSTS) { ability_available = true; }
                    }
...
void ZergMultiplayerBot::BuildArmy()
{
    ...
    size_t hydralisk_count =
      CountUnitTypeTotal(observation, UNIT_TYPEID::ZERG_HYDRALISK, UNIT_TYPEID::ZERG_EGG, ABILITY_ID::TRAIN_HYDRALISK);
    size_t roach_count =
      CountUnitTypeTotal(observation, UNIT_TYPEID::ZERG_ROACH, UNIT_TYPEID::ZERG_EGG, ABILITY_ID::TRAIN_ROACH);
    size_t corruptor_count =
      CountUnitTypeTotal(observation, UNIT_TYPEID::ZERG_CORRUPTOR, UNIT_TYPEID::ZERG_EGG, ABILITY_ID::TRAIN_CORRUPTOR);
    size_t swarmhost_count = CountUnitTypeTotal(
      observation, UNIT_TYPEID::ZERG_SWARMHOSTMP, UNIT_TYPEID::ZERG_EGG, ABILITY_ID::TRAIN_SWARMHOST);
    size_t viper_count =
      CountUnitTypeTotal(observation, UNIT_TYPEID::ZERG_VIPER, UNIT_TYPEID::ZERG_EGG, ABILITY_ID::TRAIN_VIPER);
    size_t ultralisk_count =
      CountUnitTypeTotal(observation, UNIT_TYPEID::ZERG_ULTRALISK, UNIT_TYPEID::ZERG_EGG, ABILITY_ID::TRAIN_ULTRALISK);
    size_t infestor_count =
      CountUnitTypeTotal(observation, UNIT_TYPEID::ZERG_INFESTOR, UNIT_TYPEID::ZERG_EGG, ABILITY_ID::TRAIN_INFESTOR);
    ...
    if (CountUnitType(observation, UNIT_TYPEID::ZERG_OVERSEER)
          + CountUnitType(observation, UNIT_TYPEID::ZERG_OVERLORDCOCOON)
        < 1) {
    ...
    if (larva_count > 0) {
        if (viper_count < 2) {
            if (TryBuildUnit(ABILITY_ID::TRAIN_VIPER, UNIT_TYPEID::ZERG_LARVA)) { --larva_count; }
        }
        if (CountUnitType(observation, UNIT_TYPEID::ZERG_ULTRALISKCAVERN) > 0 && !mutalisk_build_
            && ultralisk_count < 4) {
            if (TryBuildUnit(ABILITY_ID::TRAIN_ULTRALISK, UNIT_TYPEID::ZERG_LARVA)) { --larva_count; }
            // Try to build at least one ultralisk
            if (ultralisk_count < 1) { return; }
        }
    }

    if (!mutalisk_build_) {
        if (CountUnitType(observation, UNIT_TYPEID::ZERG_RAVAGER)
              + CountUnitType(observation, UNIT_TYPEID::ZERG_RAVAGERCOCOON)
            < 3) {
            TryBuildUnit(ABILITY_ID::MORPH_RAVAGER, UNIT_TYPEID::ZERG_ROACH);
        }
        if (CountUnitType(observation, UNIT_TYPEID::ZERG_LURKERMP)
              + CountUnitType(observation, UNIT_TYPEID::ZERG_LURKERMPEGG)
              + CountUnitType(observation, UNIT_TYPEID::ZERG_LURKERMPBURROWED)
            < 6) {
        ...
        if (larva_count > 0) {
            if (CountUnitType(observation, UNIT_TYPEID::ZERG_HYDRALISKDEN)
                  ...
                  > 0
                && hydralisk_count < 15) {
                if (TryBuildUnit(ABILITY_ID::TRAIN_HYDRALISK, UNIT_TYPEID::ZERG_LARVA)) { --larva_count; }
            }
        }

        if (larva_count > 0) {
            if (swarmhost_count < 1) {
                if (TryBuildUnit(ABILITY_ID::TRAIN_SWARMHOST, UNIT_TYPEID::ZERG_LARVA)) { --larva_count; }
            }
        }

        if (larva_count > 0) {
            if (infestor_count < 1) {
                if (TryBuildUnit(ABILITY_ID::TRAIN_INFESTOR, UNIT_TYPEID::ZERG_LARVA)) { --larva_count; }
            }
        }
    } else {
        if (larva_count > 0) {
            if (CountUnitType(observation, UNIT_TYPEID::ZERG_SPIRE)
                ...
                if (corruptor_count < 7) {
                    if (TryBuildUnit(ABILITY_ID::TRAIN_CORRUPTOR, UNIT_TYPEID::ZERG_LARVA)) { --larva_count; }
                }
                if (TryBuildUnit(ABILITY_ID::TRAIN_MUTALISK, UNIT_TYPEID::ZERG_LARVA)) { --larva_count; }
            }
        }
    }
    ...
    if (CountUnitType(observation, UNIT_TYPEID::ZERG_GREATERSPIRE) > 0) {
        if (CountUnitType(observation, UNIT_TYPEID::ZERG_BROODLORD)
              + CountUnitType(observation, UNIT_TYPEID::ZERG_BROODLORDCOCOON)
            < 4)
    ...
    if (!mutalisk_build_ && larva_count > 0) {
        if (roach_count < 10 && CountUnitType(observation, UNIT_TYPEID::ZERG_ROACHWARREN) > 0) {
            if (TryBuildUnit(ABILITY_ID::TRAIN_ROACH, UNIT_TYPEID::ZERG_LARVA)) { --larva_count; }
        }
    }
    size_t baneling_count = CountUnitType(observation, UNIT_TYPEID::ZERG_BANELING)
                            + CountUnitType(observation, UNIT_TYPEID::ZERG_BANELINGCOCOON);
    if (larva_count > 0) {
        if (CountUnitType(observation, UNIT_TYPEID::ZERG_ZERGLING) < 20
            && CountUnitType(observation, UNIT_TYPEID::ZERG_SPAWNINGPOOL) > 0) {
            if (TryBuildUnit(ABILITY_ID::TRAIN_ZERGLING, UNIT_TYPEID::ZERG_LARVA)) { --larva_count; }
        }
    }

    size_t baneling_target = 5;
    if (mutalisk_build_) { baneling_target = baneling_target * 2; }
    if (baneling_count < baneling_target) { TryBuildUnit(ABILITY_ID::TRAIN_BANELING, UNIT_TYPEID::ZERG_ZERGLING); }
}
...
bool ZergMultiplayerBot::TryBuildOverlord()
{
    ...
    if (observation->GetFoodUsed() < observation->GetFoodCap() - 4) { return false; }
    if (observation->GetMinerals() < 100) { return false; }
    ...
    if (observation->GetFoodUsed() < 30) {
        ...
        for (const auto &unit : units) {
            if (unit->orders.empty()) { return false; }
            if (unit->orders.front().ability_id == ABILITY_ID::TRAIN_OVERLORD) { return false; }
        }
    }
    if (larva_count > 0) { return TryBuildUnit(ABILITY_ID::TRAIN_OVERLORD, UNIT_TYPEID::ZERG_LARVA); }
    return false;
}
...
void ZergMultiplayerBot::TryInjectLarva()
{
    ...
    if (queens.empty() || hatcheries.empty()) return;
...
bool ZergMultiplayerBot::TryBuildExpansionHatch()
{
    ...
    // Don't have more active bases than we can provide workers for
    if (GetExpectedWorkers(UNIT_TYPEID::ZERG_EXTRACTOR) > max_worker_count_) { return false; }
    ...
    if (observation->GetMinerals()
        > std::min<size_t>((CountUnitType(observation, UNIT_TYPEID::ZERG_HATCHERY) * 300), 1200)) {
        return TryExpand(ABILITY_ID::BUILD_HATCHERY, UNIT_TYPEID::ZERG_DRONE);
    }
    return false;
}
...
bool ZergMultiplayerBot::BuildExtractor()
{
    ...
    for (const auto &base : bases) {
        if (base->assigned_harvesters >= base->ideal_harvesters) {
            if (base->build_progress == 1) {
                if (TryBuildGas(ABILITY_ID::BUILD_EXTRACTOR, UNIT_TYPEID::ZERG_DRONE, base->pos)) { return true; }
            }
        }
    }
    return false;
...
void ZergMultiplayerBot::OnStep()
{
    ...
    if (TryBuildDrone()) return;

    if (TryBuildOverlord()) return;

    if (observation->GetFoodArmy() < base.size() * 25) { BuildArmy(); }

    if (BuildExtractor()) { return; }

    if (TryBuildExpansionHatch()) { return; }
}
...
bool TerranMultiplayerBot::TryBuildSCV()
{
    ...
    if (observation->GetFoodWorkers() > GetExpectedWorkers(UNIT_TYPEID::TERRAN_REFINERY)) { return false; }

    for (const auto &base : bases) {
        // if there is a base with less than ideal workers
        if (base->assigned_harvesters < base->ideal_harvesters && base->build_progress == 1) {
            if (observation->GetMinerals() >= 50) { return TryBuildUnit(ABILITY_ID::TRAIN_SCV, base->unit_type); }
        }
    }
    return false;
}
...
void TerranMultiplayerBot::BuildArmy()
{
    ...
    size_t hellbat_count =
      CountUnitTypeTotal(observation, hellion_types, UNIT_TYPEID::TERRAN_FACTORY, ABILITY_ID::TRAIN_HELLION);
    hellbat_count += CountUnitTypeBuilding(observation, UNIT_TYPEID::TERRAN_FACTORY, ABILITY_ID::TRAIN_HELLBAT);

    size_t siege_tank_count =
      CountUnitTypeTotal(observation, siege_tank_types, UNIT_TYPEID::TERRAN_FACTORY, ABILITY_ID::TRAIN_SIEGETANK);
    size_t viking_count =
      CountUnitTypeTotal(observation, viking_types, UNIT_TYPEID::TERRAN_FACTORY, ABILITY_ID::TRAIN_VIKINGFIGHTER);
    size_t marine_count = CountUnitTypeTotal(
      observation, UNIT_TYPEID::TERRAN_MARINE, UNIT_TYPEID::TERRAN_BARRACKS, ABILITY_ID::TRAIN_MARINE);
    size_t marauder_count = CountUnitTypeTotal(
      observation, UNIT_TYPEID::TERRAN_MARAUDER, UNIT_TYPEID::TERRAN_BARRACKS, ABILITY_ID::TRAIN_MARAUDER);
    size_t reaper_count = CountUnitTypeTotal(
      observation, UNIT_TYPEID::TERRAN_REAPER, UNIT_TYPEID::TERRAN_BARRACKS, ABILITY_ID::TRAIN_REAPER);
    size_t ghost_count =
      CountUnitTypeTotal(observation, UNIT_TYPEID::TERRAN_GHOST, UNIT_TYPEID::TERRAN_BARRACKS, ABILITY_ID::TRAIN_GHOST);
    size_t medivac_count = CountUnitTypeTotal(
      observation, UNIT_TYPEID::TERRAN_MEDIVAC, UNIT_TYPEID::TERRAN_STARPORT, ABILITY_ID::TRAIN_MEDIVAC);
    size_t raven_count =
      CountUnitTypeTotal(observation, UNIT_TYPEID::TERRAN_RAVEN, UNIT_TYPEID::TERRAN_STARPORT, ABILITY_ID::TRAIN_RAVEN);
    size_t battlecruiser_count = CountUnitTypeTotal(
      observation, UNIT_TYPEID::TERRAN_MEDIVAC, UNIT_TYPEID::TERRAN_STARPORT, ABILITY_ID::TRAIN_BATTLECRUISER);
    size_t banshee_count = CountUnitTypeTotal(
      observation, UNIT_TYPEID::TERRAN_MEDIVAC, UNIT_TYPEID::TERRAN_STARPORT, ABILITY_ID::TRAIN_BANSHEE);

    if (!mech_build_
        && CountUnitType(observation, UNIT_TYPEID::TERRAN_GHOSTACADEMY)
        ...
        if (!nuke_built) {
            ...
            if (!ghosts.empty()) {
                ...
                for (const auto &ability : abilities.abilities) {
                    if (ability.ability_id == ABILITY_ID::EFFECT_NUKECALLDOWN) { nuke_built = true; }
                }
            }
        }
    }
                    ...
                    if (!mech_build_) {
                        if (CountUnitType(observation, UNIT_TYPEID::TERRAN_FUSIONCORE) > 0) {
                            if (starport->orders.empty() && battlecruiser_count < 2) {
                                ...
                                if (battlecruiser_count < 1) { return; }
                            }
                        }
        ...
        for (const auto &barrack : barracks) {
            if (observation->GetUnit(barrack->add_on_tag) == nullptr) {
                if (!mech_build_) {
                    ...
                    } else if (barrack->orders.empty() && observation->GetMinerals() > 1000
        ...
        for (const auto &factory : factorys) {
            if (observation->GetUnit(factory->add_on_tag) == nullptr) {
                ...
                if (observation->GetUnit(factory->add_on_tag)->unit_type == UNIT_TYPEID::TERRAN_FACTORYREACTOR) {
                    ...
                    if (mech_build_ && factory->orders.empty()
                        && CountUnitType(observation, UNIT_TYPEID::TERRAN_ARMORY) > 0) {
...
void TerranMultiplayerBot::BuildOrder()
{
    ...
    for (const auto &barrack : barracks) {
        if (!barrack->orders.empty() || barrack->build_progress != 1) { continue; }
        if (observation->GetUnit(barrack->add_on_tag) == nullptr) {
    ...
    for (const auto &starport : starports) {
        if (!starport->orders.empty()) { continue; }
        if (observation->GetUnit(starport->add_on_tag) == nullptr) {
            if (mech_build_) {
...
bool TerranMultiplayerBot::TryBuildStructureRandom(AbilityID ability_type_for_structure, UnitTypeID unit_type)
{
    ...
    float distance = std::numeric_limits<float>::max();
    for (const auto &u : units) {
        if (u->unit_type == UNIT_TYPEID::TERRAN_SUPPLYDEPOTLOWERED) { continue; }
        float d = Distance2D(u->pos, build_location);
        if (d < distance) { distance = d; }
    }
    if (distance < 6) { return false; }
...
void TerranMultiplayerBot::ManageArmy()
{
    ...
    Units army = observation->GetUnits(Unit::Alliance::Self, IsArmy(observation));
    int wait_til_supply = 100;
    if (mech_build_) { wait_til_supply = 110; }
    ...
    for (const auto &unit : army) {
        if (enemy_units.empty() && observation->GetFoodArmy() < wait_til_supply) {
            ...
            case UNIT_TYPEID::TERRAN_WIDOWMINE: {
                ...
                if (distance < 6) { Actions()->UnitCommand(unit, ABILITY_ID::BURROWDOWN); }
                break;
            }
                ...
                if (stim_researched_ && !unit->orders.empty()) {
                    if (unit->orders.front().ability_id == ABILITY_ID::ATTACK) {
                        ...
                        for (const auto &buff : unit->buffs) {
                            if (buff == BUFF_ID::STIMPACK) { has_stimmed = true; }
                        }
            ...
            case UNIT_TYPEID::TERRAN_MEDIVAC: {
                ...
                if (unit->orders.empty()) {
                    ...
                    if (!bio_units.empty()) { Actions()->UnitCommand(unit, ABILITY_ID::ATTACK, bio_units.front()); }
                }
                break;
            }
            ...
            case UNIT_TYPEID::TERRAN_RAVEN: {
                ...
                if (unit->orders.empty()) { Actions()->UnitCommand(unit, ABILITY_ID::ATTACK, army.front()->pos); }
                break;
            }
            default: {
                AttackWithUnit(unit, observation);
            ...
            case UNIT_TYPEID::TERRAN_MEDIVAC: {
                Units bio_units = observation->GetUnits(Unit::Self, IsUnits(bio_types));
                if (unit->orders.empty()) { Actions()->UnitCommand(unit, ABILITY_ID::ATTACK, bio_units.front()->pos); }
                break;
            }
            default:
                ScoutWithUnit(unit, observation);
...
bool TerranMultiplayerBot::TryBuildExpansionCom()
{
    ...
    Units bases = observation->GetUnits(Unit::Alliance::Self, IsTownHall());
    // Don't have more active bases than we can provide workers for
    if (GetExpectedWorkers(UNIT_TYPEID::TERRAN_REFINERY) > max_worker_count_) { return false; }
    // If we have extra workers around, try and build another Hatch.
...
bool TerranMultiplayerBot::BuildRefinery()
{
    ...
    for (const auto &base : bases) {
        if (base->assigned_harvesters >= base->ideal_harvesters) {
            if (base->build_progress == 1) {
                if (TryBuildGas(ABILITY_ID::BUILD_REFINERY, UNIT_TYPEID::TERRAN_SCV, base->pos)) { return true; }
            }
        }
    }
    return false;
...
void TerranBot::ScoutWithMarines()
{
    ...
    for (const auto &unit : units) {
        ...
        if (unit_type != UNIT_TYPEID::TERRAN_MARINE) continue;

        if (!unit->orders.empty()) continue;
        ...
        if (FindEnemyPosition(target_pos)) { Actions()->UnitCommand(unit, ABILITY_ID::SMART, target_pos); }
    }
}
...
bool TerranBot::TryBuildSupplyDepot()
{
    ...
    // If we are not supply capped, don't build a supply depot.
    if (observation->GetFoodUsed() <= observation->GetFoodCap() - 2) return false;
...
bool TerranBot::TryBuildBarracks()
{
    ...
    if (CountUnitType(observation, UNIT_TYPEID::TERRAN_SCV) < TargetSCVCount) return false;

    // One build 1 barracks.
    if (CountUnitType(observation, UNIT_TYPEID::TERRAN_BARRACKS) > 0) return false;
...
bool TerranBot::TryBuildMarine() { return TryBuildUnit(ABILITY_ID::TRAIN_MARINE, UNIT_TYPEID::TERRAN_BARRACKS); }
...
void TerranBot::OnStep()
{
    ...
    if (TryBuildBarracks()) return;

    // Just keep building marines if possible.
    if (TryBuildMarine()) return;
}
...
void MarineMicroBot::OnStep()
{
    ...
    if (!GetPosition(UNIT_TYPEID::TERRAN_MARINE, Unit::Alliance::Self, mp)) { return; }

    if (!GetPosition(UNIT_TYPEID::ZERG_ZERGLING, Unit::Alliance::Enemy, zp)) { return; }

    if (!GetNearestZergling(mp)) { return; }
    ...
    for (const auto &u : units) {
        ...
        case UNIT_TYPEID::TERRAN_MARINE: {
            if (!move_back_) {
                ...
                if (Distance2D(mp, backup_target_) < 1.5f) { move_back_ = false; }
